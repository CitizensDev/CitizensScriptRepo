<form id='post' action='' method='post' accept-charset='UTF-8'>
	<fieldset>
		...
		<div class="well well-large">
			...
			<input class="input-large" name='tags' type="text"{if $tags} value="{$tags}"{/if} /><br>{if $tagError}</div>{/if}
			<small class="muted">Separate tags with a comma.</small>
			...
			<div class="btn-group" data-toggle="buttons-radio">
				<input onclick="this.form.elements['typeOfScript'].value = '1';" id="type2" type="button" class="btn" value="Citizens Script" />
				<input onclick="this.form.elements['typeOfScript'].value = '2';" id="type2" type="button" class="btn" value="Denizen Script" />
				<input onclick="this.form.elements['typeOfScript'].value = '3';" id="type2" type="button" class="btn" value="Uhh... Script" />
			</div>{if $typeError}</div>{/if}
			<small class="muted">Pick one!</small>
			<br><br><br>
