...
class ScriptRepo{
    ...
    private function postScript($postData, $oldID=false){
    ...
    private function isActiveUser($username){
        $username = $this->databaseHandle->real_escape_string($username);
        $result = $this->queryDatabase("SELECT * FROM repo_users WHERE username='$username'");
        $row = $result->fetch_assoc();
    ...
    private function getResults($queryHandle, $numberPerPage, $pageNumber){
        $outputArray = array();
        $count = 0;
